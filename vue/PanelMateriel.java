...
public class PanelMateriel extends PanelDeBase implements ActionListener {
	...
	public PanelMateriel() {
		...
		this.panelForm.add(new JLabel("   Quantite du materiel"));
		...
		String entets[] = { "ID materiels", "Nom", "Marque", "Poids", "Capacite", "Taille", "Quantite"};
		...
		this.uneTable.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (nbclic == 2) {
					...
					String quantite = unTableau.getValueAt(numLigne,6).toString();
	...
	public Object[][] getLesDonnees(String mot) {
		...
		for(Materiels unMateriel : lesMateriels) {
			...
			matrice[i][6] = unMateriel.getQuantite();
			i++;
		}
		return matrice;
	}
	...
	public Materiels saisirMateriel() {
		...
		if (nom.equals("")) {
			JOptionPane.showMessageDialog(null, 
			         "Veuillez Vérifier Le Champ Nom", 
			         "Tout Les Champs Ne son pas saisie", 
		...
		if (marque.equals("")) {
			JOptionPane.showMessageDialog(null, 
			         "Veuillez Vérifier Le Champ Marque", 
			         "Tout Les Champs Ne son pas saisie", 
			         JOptionPane.INFORMATION_MESSAGE, 
			         null);
		} else {
		...
		if (poids.equals("")) {
			this.txtPoids.setBackground(Color.red);
		} else {
		...
		if (taille.equals("")) {
			this.txtTaille.setBackground(Color.red);
		} else {
			this.txtTaille.setBackground(Color.white);
		}
	
		 if (!nom.equals("") && !marque.equals("") && !poids.equals("") && !capacite.equals("") && !taille.equals("") && !quantite.equals("")) {
			unMateriel = new Materiels (nom, marque, poids, capacite,taille,quantite);
		} else {
			return null;
		}
